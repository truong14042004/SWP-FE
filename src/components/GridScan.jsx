...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  float alpha = clamp(max(lineVis, combinedPulse), 0.0, 1.0) * fade;
...
export const GridScan = ({
  ...
  useEffect(() => {
    ...
    const onMove = e => {
      ...
      // Check if mouse is inside the bounding box of the element (plus small padding)
      const padding = 100;
      const isInside = 
      ...
      if (isInside) {
        ...
        lookTarget.current.set(
          Math.max(-1.5, Math.min(1.5, nx)),
          Math.max(-1.5, Math.min(1.5, ny))
        );
      } else {
        if (!leaveTimer) {
    ...
    <div ref={containerRef} className={`gridscan${className ? ` ${className}` : ''}`} style={style}>
      {showPreview && (
        <div className="gridscan__preview">
          <video ref={videoRef} videoRef={videoRef} muted playsInline autoPlay className="gridscan__video" />
